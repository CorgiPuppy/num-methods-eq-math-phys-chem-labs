...
\begin{document}
		...
		\begin{tikzpicture}[node distance=2cm]
			...
			\node (in1) [process, below of=start] {
				\makecell{
					...
					$u_{i, j}^{0} = \sin{((i-1)h_{x}(j-1)h_{y})}$
				}
			};
			...
			\node (pr1) [process, below of=dec, yshift=-1cm] {
				\makecell{
					...
					3) $u_{N_{x}, j}^{n+\frac{1}{2}} = 2(n+\frac{1}{2})\Delta t(j-1)h_{y}$; \\
			...
			\node (pr2) [process, below of=pr1, yshift=-1cm] {
				\makecell{
					цикл по $i=1..N_{x}$ \\
					$u_{i, 1}^{n+\frac{1}{2}} = u_{i, 2}^{n+\frac{1}{2}}$ \\
					$u_{i, N_{y}}^{n+\frac{1}{2}} = 1$
				}
			};
			\node (pr3) [process, below of=pr2, yshift=-2cm] {
				\makecell{
					цикл по $i=2..N_{x}-1$ \\
					1) $\tilde{\alpha}_{1} = 1$, $\tilde{\beta}_{1} = 0$; \\
			...
			\node (pr4) [process, below of=pr3, yshift=-1cm] {
				\makecell{
					...
					$u_{N_{x}, j}^{n+1} = 2(n+1)\Delta t(j-1)h_{y}$
				}
			};
	...
	\large
	Составить алгоритм (блок-схему) расчёта схемы \eqref{eq:variableDirectionsScheme2}:
	\small
	\tikzstyle{start} = [circle, draw=black!60, fill=white!5, very thick, minimum size=10mm]
	\tikzstyle{stop} = [circle, color=black!60, fill=black!60, very thick, minimum size=10mm]
	\tikzstyle{process} = [rectangle, minimum width=3cm, minimum height=1cm, text centered, draw=black]
	...
	\large
	Составить алгоритм (блок-схему) расчёта для схемы \eqref{eq:explicit3}:
		...
		\begin{tikzpicture}[node distance=2cm]
			...
			\node (pr1) [process, below of=in2, yshift=-0.5cm] {
				\makecell{
					...
					$u_{i, j}^{n+1} = \frac{0,2u_{i+1, j}^{n} + 0,2u_{i-1, j}^{n} + 0,2u_{i, j+1}^{n} + 0,2u_{i, j-1}^{n} + h^{2}3e^{(i-1)h_{x}(j-1)h_{y}}}{4*0,2}$
				}
			};
			\node (pr2) [process, below of=pr1, yshift=-1cm] {
				\makecell{
			...
			\node (pr3) [process, below of=pr2, yshift=-1cm] {
				\makecell{
					цикл по $i=1..N_{x}$: \\
					$u_{i, 1}^{n+1} = 0$ \\
					$u_{i, N_{y}}^{n+1} = (i-1)h_{x}$
				}
			};
			\node (dec) [decision, below of=pr3, yshift=-2cm] {
				$\lvert \lvert u^{n+1} - u^{n} \rvert \rvert \le \epsilon$
			};
			\node (stop) [stop, right of=dec, xshift=2cm] {End};
			\node (pr4) [process, below of=dec, yshift=-2cm] {
				$n = n + 1$
			};
			\draw [arrow] (start) -- (in1);
			\draw [arrow] (in1) -- (in2);
			...
			\draw [arrow] (pr2) -- (pr3);
			\draw [arrow] (pr3) -- (dec);
			\draw [arrow] (dec) -- node[anchor=west] {нет} (pr4);
			\draw [arrow] (dec) -- node[anchor=south] {да} (stop);
			\draw [arrow] (pr4.west) -- ++(left:8.2cm) -| +(up:14cm) -- (pr1.west);
		\end{tikzpicture}
	\end{center}

	\subsection*{Задание 18}
	\addcontentsline{toc}{subsection}{Задание 1k}
	\large
